...
async function generateSubdomainSuggestions(
	...
	for (const strategy of strategies) {
		const candidates = strategy().filter(
			(c) =>
				c.length >= MIN_SUBDOMAIN_LENGTH && c.length <= MAX_SUBDOMAIN_LENGTH,
		);
	...
	if (suggestions.length < maxSuggestions) {
		...
		for (
			let counter = 1;
			counter <= MAX_NUMBERED_ATTEMPTS &&
			numberedCandidates.length < maxSuggestions - suggestions.length;
			counter++
		) {
