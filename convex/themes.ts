...
export const applyThemeToBusiness = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const hasAccess = theme.isPublic || 
...
export const duplicateTheme = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const newThemeId = await ctx.db.insert("themes", {
      name: args.name,
      description: sourceTheme.description ? `Copy of ${sourceTheme.description}` : undefined,
      isPreset: false,
  ...
  handler: async (ctx, args) => {
    ...
    if (business.theme.colorScheme) {
      ...
      const matchingPreset = presets.find(p => 
        p.presetId === business.theme?.colorScheme?.toLowerCase()
      );
