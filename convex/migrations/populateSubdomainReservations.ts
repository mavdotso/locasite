		...
		for (const domain of domains) {
			try {
				...
				if (existingReservation) {
					if (
						existingReservation.domainId &&
						existingReservation.domainId !== domain._id
					) {
						const errorMsg = `Reservation conflict on ${domain.subdomain}: belongs to ${existingReservation.domainId}, expected ${domain._id}`;
					...
					const needsPatch =
						...
						existingReservation.expiresAt !== undefined;
					...
					if (needsPatch) {
						...
						console.log(
							`Updated reservation for subdomain: ${domain.subdomain}`,
						);
					}
