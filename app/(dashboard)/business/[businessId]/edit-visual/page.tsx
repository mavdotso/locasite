...
interface EditVisualPageProps {
  params: Promise<{
    businessId: string;
  }>;
}

export default function EditVisualPage({ params }: EditVisualPageProps) {
    ...
    try {
      ...
      if (parsed.sections) {
        initialData = {
          ...
          components: parsed.sections.map((section: any, index: number) => ({
            id: `component-${index}`,
...
function mapSectionPropsToComponentProps(section: any): Record<string, unknown> {
  const { type, hidden, ...props } = section;
