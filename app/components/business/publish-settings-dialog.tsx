...
export function PublishSettingsDialog({ 
  ...
  }, [seoSettings.pageTitle, seoSettings.seoTitle, seoSettings.seoDescription, businessName, socialSettings.ogTitle, socialSettings.ogDescription]);
  ...
  const handlePublish = async () => {
    if (!validateSettings()) {
      ...
      if (validationErrors.domain) {
        setActiveTab("domain");
      } else if (validationErrors.pageTitle || validationErrors.seoTitle || validationErrors.seoDescription) {
        setActiveTab("seo");
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure your website&apos;s domain, SEO, and social media settings before publishing.
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="seo" className="flex items-center gap-2">
              ...
              SEO
              {(validationErrors.pageTitle || validationErrors.seoTitle || validationErrors.seoDescription) && (
                <AlertCircle className="h-3 w-3 text-destructive" />
              )}
            </TabsTrigger>
          ...
          <TabsContent value="seo" className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  setSeoSettings(prev => ({ ...prev, pageTitle: e.target.value }));
                  setValidationErrors(prev => ({ ...prev, pageTitle: "" }));
                }}
                placeholder="My Business"
                disabled={isPublishing}
                className={validationErrors.pageTitle ? "border-destructive" : ""}
              />
              {validationErrors.pageTitle && (
                <p className="text-xs text-destructive">{validationErrors.pageTitle}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  disabled={isPublishing}
                  className={validationErrors.seoTitle ? "border-destructive" : ""}
                />
                <span className={cn(
                  "absolute right-2 top-1/2 -translate-y-1/2 text-xs",
                  seoSettings.seoTitle.length > 60 ? "text-destructive" : "text-muted-foreground"
                )}>
                  {seoSettings.seoTitle.length}/60
                </span>
              </div>
              {validationErrors.seoTitle && (
                <p className="text-xs text-destructive">{validationErrors.seoTitle}</p>
              )}
              <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Textarea
                  ...
                  onChange={(e) => {
                    setSeoSettings(prev => ({ ...prev, seoDescription: e.target.value }));
                    setValidationErrors(prev => ({ ...prev, seoDescription: "" }));
                  }}
                  placeholder="We provide the best service in town..."
                  rows={3}
                  disabled={isPublishing}
                  className={validationErrors.seoDescription ? "border-destructive" : ""}
                />
                <span className={cn(
                  "absolute right-2 bottom-2 text-xs",
                  seoSettings.seoDescription.length > 160 ? "text-destructive" : "text-muted-foreground"
                )}>
                  {seoSettings.seoDescription.length}/160
                </span>
              </div>
              {validationErrors.seoDescription && (
                <p className="text-xs text-destructive">{validationErrors.seoDescription}</p>
              )}
              <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSeoSettings(prev => ({ ...prev, seoKeywords: e.target.value }))}
          ...
          <TabsContent value="social" className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSocialSettings(prev => ({ ...prev, ogTitle: e.target.value }))}
                placeholder="My Business"
                disabled={isPublishing}
              />
              <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) => setSocialSettings(prev => ({ ...prev, ogDescription: e.target.value }))}
            ...
            <div className="space-y-2">
              ...
              {socialSettings.ogImage && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSocialSettings(prev => ({ ...prev, ogImage: "" }))}
                  className="w-full"
                  disabled={isPublishing}
                >
              ...
              <p className="text-xs text-muted-foreground">
                Recommended size: 1200x630px. Appears when shared on social media.
              </p>
            </div>
          </TabsContent>
        </Tabs>
