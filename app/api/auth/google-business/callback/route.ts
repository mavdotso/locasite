  ...
  try {
    ...
    await fetchMutation(api.businesses.updateGoogleBusinessAuth, {
      ...
      googleBusinessAuth: {
        ...
        accounts: accounts.map((account: { name: string; accountName: string; type: string; verificationState?: string }) => ({
    ...
    return NextResponse.redirect(
      new URL(`/dashboard/businesses/${businessId}/verify?success=true`, request.url)
    );
  } catch (error) {
